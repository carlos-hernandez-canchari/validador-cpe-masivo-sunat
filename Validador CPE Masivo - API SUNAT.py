...
while True:
    ...
    while fila <= ultima_fila:
        ...
        if not str(monto).replace(".", "").replace(",", "").replace("-", "").replace(" ", "").isdigit():
        ...
        elif len(ruc) != 11 or not ruc.isdigit() or ruc[:2] not in ["10", "15", "20"]:
            ...
            try:
                ...
                if resp.status_code == 200 and resp.json().get("success", False):
                    ...
                    h = ESTADO_CP.get(estado_cp, "DESCONOCIDO")
                    ...
                    # Si el estado del comprobante es desconocido, tratar como error y reintentar
                    if h == "DESCONOCIDO":
